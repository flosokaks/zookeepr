...
<p>I allow ${ h.klf_info["event_parent_organisation"] } to record my talk.</p>

<p>${ allow(c.proposal.video_release) } ${ h.klf_info["event_parent_organisation"] } to release any
recordings of my presentations, tutorials and minconfs under the <a href="${ h.klf_info["media_license_url"] }">${ h.klf_info["media_license_name"] }</a></p>

<p>${ allow(c.proposal.slides_release) } ${ h.klf_info["event_parent_organisation"] } to release any other material (such as slides) from my presentations, tutorials and minconfs under the <a href="${ h.klf_info["media_license_url"] }">${ h.klf_info["media_license_name"] }</a>
</p>


% if c.proposal.video_release or c.proposal.slides_release:
<p>I confirm that I have the authority to allow
${ h.klf_info["event_parent_organisation"] } to release the above material.
