...
${ h.hidden('payment_reference', h.klf_info['event_shortname'] + ' i-' + str(c.payment.invoice.id) + ' p-' + str(c.payment.id)) }
${ h.hidden('vendor_name', 'linux') }
${ h.hidden('cards_accepted', 'VISA,MASTERCARD,AMEX') }
${ h.hidden('gst_rate', '10') }
${ h.hidden('gst_added', 'TRUE') }
${ h.hidden(h.klf_info['event_name'] + ' Registration', h.integer_to_currency(c.payment.amount, unit='' )) }
${ h.hidden('hidden_fields', 'submit') }
${ h.hidden('receipt_address', c.payment.invoice.person.email_address) }
${ h.hidden('return_link_text', 'Return to ' + h.klf_info['event_name']) }
