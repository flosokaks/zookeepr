...
class PersonaValidator(validators.FancyValidator):
    def validate_python(self, values, state):
        ...
        if c.person is None:
            if not klf_info['account_creation']:
...
class PersonController(BaseController): #Read, Update, List
    ...
    def _redirect_user_optimally(self):
        ...
        if klf_info['conference_status'] == 'open':
            redirect_to(controller='registration', action='new')
        elif klf_info['cfp_status'] == 'open':
    ...
    def _new(self):
        ...
        if klf_info['account_creation']:
            ...
            if klf_rego['confirm_email_address'] == 'no':
