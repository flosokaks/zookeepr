...
% if len(h.klf_info['cfp_miniconf_list']) > 1 and c.proposal.proposal_type_id is not 2:
  <div id="q3">
    <p class="label"><b>What miniconf would this talk be most suitable for, if it's not accepted?</b></p>
    <p class="entries">${ h.select('review.miniconf', None, [ (mc, mc) for mc in h.klf_info['cfp_miniconf_list']] ) }</p>
  </div>
% else:
